...
import airbnbRoutes from "./back_end/routes/airbnb.routes.js"; // Rutas para el hotel (habitaciones y usuarios)
...
app.use("/api", airbnbRoutes); // Monta las rutas relacionadas con airbnb bajo el prefijo "/api"
...
app.post("/upload", upload.single("imagen"), (req, res) => {
  console.log(req.file); // Imprime la información del archivo cargado en la consola
  res.status(200).json({
...
mongoose
  .connect(process.env.MONGO_URI) // Intenta conectar a la base de datos MongoDB utilizando la URI definida en las variables de entorno
